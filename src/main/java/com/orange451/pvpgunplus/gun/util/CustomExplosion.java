...
public class CustomExplosion {
	...
	private void breakBlockAndParticles() {
		...
		this.world.addParticle(particle, this.posX, this.posY, this.posZ, 1, 0, 0, new int[0]);
		...
		for (BlockPosition pos : blocks) {
			...
			if (showParticles) {
				...
				offY *= idk;
				offZ *= idk;
				this.world.addParticle(EnumParticle.EXPLOSION_NORMAL, (partX + this.posX * 1.0D) / 2.0D, (partY + this.posY * 1.0D) / 2.0D, (partZ + this.posZ * 1.0D) / 2.0D, offX, offY, offZ, new int[0]);
				this.world.addParticle(EnumParticle.SMOKE_NORMAL, partX, partY, partZ, offX, offY, offZ, new int[0]);
			}
