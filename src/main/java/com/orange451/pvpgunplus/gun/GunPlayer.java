...
public class GunPlayer
{
    ...
    public boolean isAimedIn()
    {
        ...
        if (controller.hasPotionEffect(PotionEffectType.SLOW))
            return true;
    ...
    public Gun getGun(String name)
    {
        ...
        {
            Gun check = guns.get(i);
            if (check.getName().toLowerCase().equals(name.toLowerCase()) || check.getFilename().toLowerCase().equals(name.toLowerCase()))
                return check;
        }
    ...
    public void damageByLava(EntityDamageEvent e) {
    	...
    	Sync.get().delay(1).run(() -> PVPGunPlus.resetPlayerDamage((LivingEntity) getPlayer(), 0));
